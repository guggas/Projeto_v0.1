...
public class FicheiroDeTexto {
	...
	public void escreveLinha(String linha) throws IOException{
		fW.write(linha,0,linha.length());		//1 - string a escrever; 2 - índice de início de escrita(inclusive); 3- índice do final (primeiro índice não incluído);
		fW.newLine();
	}
	
	//Método para escrever um número inteiro no ficheiro
	//Recebe o número a escrever
	public void escreveNumero(int num) throws IOException{  //conerte int recebido em String para devolver
		String st = "";
		st = st.valueOf(num);
		escreveLinha(st);
	}
